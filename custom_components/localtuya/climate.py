        ...
        if self._config.get(CONF_HEURISTIC_ACTION, False):
            if self._hvac_mode == HVACMode.HEAT:
                if self._current_temperature < (
                    ...
                    self._hvac_action = HVACAction.HVAC_HEAT
                if self._current_temperature == (
                    self._target_temperature - self._precision
                ):
                    if self._hvac_action == HVACAction.HVAC_HEAT:
                        self._hvac_action = HVACAction.HVAC_HEAT
                    if self._hvac_action == HVACAction.HVAC_IDLE:
                        self._hvac_action = HVACAction.HVAC_IDLE
                if (
